...
var ipb=document.getElementById("import");
var epb=document.getElementById("export");
...
function savePage(filename, htmlText) {
  ...
  a.href ='data:text/plain;charset=utf-8,' + encodeURIComponent(htmlText);
  a.download = filename+'__'+(getAdjCurrDateTimeOffset(true).split('T')[0].split('-').join('_'));
...
epb.onclick=()=>{
    let enc=LZString.compressToEncodedURIComponent(JSON.stringify([...document.querySelectorAll('textarea')].map(x=>{return x.value;})));
    savePage('Code_injections',enc);
}

ipb.oninput=(e)=>{
	 fr.onload = function () {
		str = this.result;
        let setts=JSON.parse(LZString.decompressFromEncodedURIComponent(str));
...
function create_sct(){
		...
		sc.innerHTML='<textarea placeholder="URL (Use asterisks with slashes)" style="box-shadow: 0 0 0px 1px black; border-width: 0px; width:32.5%;"></textarea><textarea placeholder="CSS" style="box-shadow: black 0px 0px 0px 1px;border-width: 0px;width: 32.5%;margin-left: 0.16%;"></textarea><textarea placeholder="()=>{ //ONLY WRITE FUNCTION BODY IN THIS TEXT BOX!\n}" style="box-shadow: black 0px 0px 0px 1px;border-width: 0px;width: 32.5%;margin-left: 0.16%;"></textarea><br><br>';
		return sc;
}
...
let sct2=[...document.querySelectorAll('SECTION.site_sets')];
	if (
            ((u.value!="" && (f.value!="" || f2.value!="") )&&(sct2.length==1))||
            ((u.value!="" && (f.value!="" || f2.value!="") )&&(init!=1)&&(sct2.length>1))
        ){
		forceNewSct(sci);
